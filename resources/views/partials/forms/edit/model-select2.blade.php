...
<div id="{{ $fieldname }}" class="form-group{{ $errors->has($fieldname) ? ' has-error' : '' }} ">
    ...
    <div class="col-md-7{{  ((isset($field_req) && ($required =='true'))) ?  ' required' : '' }}">
        <select class="js-data-ajax2" data-endpoint="models" data-placeholder="{{ trans('general.select_model') }}" name="{{ $fieldname }}" style="width: 100%" id="model_select_id" aria-label="{{ $fieldname }}">
