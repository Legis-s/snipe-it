...
<div id="{{ $fieldname }}" class="form-group{{ $errors->has($fieldname) ? ' has-error' : '' }} ">
    ...
    <div class="col-md-7{{  ((isset($required) && ($required =='true'))) ?  ' required' : '' }}">
        <select data-endpoint="consumables" data-placeholder="Выберите расходник" name="{{ $fieldname }}" style="width: 100%" id="consumable_select_id" aria-label="{{ $fieldname }}">
    ...
    <div class="col-md-1 col-sm-1 text-left">
        @can('create', \App\Models\Consumable::class)
            @if ((!isset($hide_new)) || ($hide_new!='true'))
                <a href='{{ route('modal.show', 'consumable') }}' data-toggle="modal"  data-target="#createModal" data-select='consumable_select_id' class="btn btn-sm btn-primary">New</a>
