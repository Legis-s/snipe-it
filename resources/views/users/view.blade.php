              ...
              <i class="fas fa-map-marker-alt fa-2x"></i></span>
                    ...
                    @can('update', $user)
                        <li class="dropdown pull-right">
                            <a class="dropdown-toggle" data-toggle="dropdown" href="#">
                                ...
                                <span class="hidden-lg hidden-md hidden-xl"><i class="fas fa-cog fa-2x"
                ...
                {{ trans('button.actions') }}
                            ...
                            <ul class="dropdown-menu">
                                <li>
                                    <a href="{{ route('users.edit', $user->id) }}">{{ trans('admin/users/general.edit') }}</a>
                                </li>
                                <li>
                                    <a href="{{ route('users.clone.show', $user->id) }}">{{ trans('admin/users/general.clone') }}</a>
                                </li>
                                @if ((Auth::user()->id !== $user->id) && (!config('app.lock_passwords')) && ($user->deleted_at==''))
                                    <li>
                                        <a href="{{ route('users.destroy', $user->id) }}">{{ trans('button.delete') }}</a>
                                    </li>
                                @endif
                            </ul>
                        </li>
                    ...
                    @can('update', \App\Models\User::class)
                        <li class="pull-right"><a href="#" data-toggle="modal" data-target="#uploadFileModal">
                                ...
                                <span class="hidden-lg hidden-md hidden-xl"><i class="fas fa-paperclip fa-2x"
                                                                               aria-hidden="true"></i></span>
                            ...
                            <div class="col-md-3 col-xs-12 col-sm-push-9">
                                ...
                                <div class="col-md-12 text-center">
                                    ...
                                    @if (($user->isSuperUser()) || ($user->hasAccess('admin')))
                                        ...
                                        <div class="{{  ($user->isSuperUser()) ? 'text-danger' : ' text-orange'}}"
                                             style="font-weight: bold">{{  ($user->isSuperUser()) ? 'superadmin' : 'admin'}}</div>
                                    @endif


                                </div>
                                <div class="col-md-12 text-center">
                                    <img src="{{ $user->present()->gravatar() }}" class=" img-thumbnail hidden-print"
                                         style="margin-bottom: 20px;" alt="{{ $user->present()->fullName() }}">
                                </div>


                                @can('update', $user)
                                    <div class="col-md-12">
                                        <a href="{{ route('users.edit', $user->id) }}" style="width: 100%;"
                                           class="btn btn-sm btn-primary hidden-print">{{ trans('admin/users/general.edit') }}</a>
                                    </div>
                                @endcan

                                @can('create', $user)
                                    <div class="col-md-12" style="padding-top: 5px;">
                                        <a href="{{ route('users.clone.show', $user->id) }}" style="width: 100%;"
                                           class="btn btn-sm btn-primary hidden-print">{{ trans('admin/users/general.clone') }}</a>
                                    </div>
                                @endcan

                                @can('view', $user)
                                    <div class="col-md-12" style="padding-top: 5px;">
                                        <a href="{{ route('users.print', $user->id) }}" style="width: 100%;"
                                           class="btn btn-sm btn-primary hidden-print" target="_blank"
                                           rel="noopener">{{ trans('admin/users/general.print_assigned') }}</a>
                                    </div>
                                @endcan
                                ...
                                @can('view', $user)
                                    <div class="col-md-12" style="padding-top: 5px;">
                                        @if(!empty($user->email))
                                            <form action="{{ route('users.email',['userId'=> $user->id]) }}"
                                                  method="POST">
                                                {{ csrf_field() }}
                                                <button style="width: 100%;" class="btn btn-sm btn-primary hidden-print"
                                                        rel="noopener">{{ trans('admin/users/general.email_assigned') }}</button>
                                            </form>
                                        @else
                                            <button style="width: 100%;" class="btn btn-sm btn-primary hidden-print"
                                                    rel="noopener" disabled
                                                    title="{{ trans('admin/users/message.user_has_no_email') }}">{{ trans('admin/users/general.email_assigned') }}</button>
                                        @endif
                                    </div>
                                @endcan
                                ...
                                @can('update', $user)
                                    @if (($user->activated == '1') && ($user->email != '') && ($user->ldap_import == '0'))
                                        <div class="col-md-12" style="padding-top: 5px;">
                                            <form action="{{ route('users.password',['userId'=> $user->id]) }}"
                                                  method="POST">
                                                {{ csrf_field() }}
                                                <button style="width: 100%;"
                                                        class="btn btn-sm btn-primary hidden-print">{{ trans('button.send_password_link') }}</button>
                                            </form>
                                        </div>
                                    @endif
                                @endcan
                                    ...
                                    @if ($user->deleted_at=='')
                                        <div class="col-md-12" style="padding-top: 30px;">
                                            <form action="{{route('users.destroy',$user->id)}}" method="POST">
                                                ...
                                                {{ method_field("DELETE")}}
                                                <button style="width: 100%;"
                                                        class="btn btn-sm btn-warning hidden-print">{{ trans('button.delete')}}</button>
                                            </form>
                                        </div>
                                        <div class="col-md-12" style="padding-top: 5px;">
                                            <form action="{{ route('users/bulkedit') }}" method="POST">
                                                ...
                                                <input type="hidden" name="ids[{{ $user->id }}]"
                                                       value="{{ $user->id }}"/>
                                                <button style="width: 100%;"
                                                        class="btn btn-sm btn-danger hidden-print">{{ trans('button.checkin_and_delete') }}</button>
                                            </form>
                                        </div>
                                    @else
                                        <div class="col-md-12" style="padding-top: 5px;">
                                            <form method="POST" action="{{ route('users.restore.store', $user->id) }}">
                                                @csrf
                                                <button style="width: 100%;"
                                                        class="btn btn-sm btn-warning hidden-print">{{ trans('button.restore') }}</button>
                                            </form>
                                        </div>
                                    @endif
                                @endcan
                                ...
                                <div class="row-new-striped">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-9">
                                            ...
                                            @if ($user->isSuperUser())
                                                <label class="label label-danger"><i class="fas fa-crown"
                                                                                     title="superuser"></i></label>
                                                &nbsp;
                                            @elseif ($user->hasAccess('admin'))
                                                <label class="label label-warning"><i class="fas fa-crown"
                                                                                      title="admin"></i></label>&nbsp;
                                            @endif
                                            {{ $user->username }}
                                                ...
                                                @foreach ($user->groups as $group)
                                                    ...
                                                    @can('superadmin')
                                                        <a href="{{ route('groups.show', $group->id) }}"
                                                           class="label label-default">{{ $group->name }}</a>
                                                    @else
                                                        {{ $group->name }}
                                                    @endcan
                                    ...
                                    @if ($user->favoriteLocation)
                                        ...
                                        <div class="row">
                                            <div class="col-md-3">
                                                {{ trans('admin/users/table.favorite_location') }}
                                            </div>
                                            <div class="col-md-9">
                                                {{ link_to_route('locations.show', $user->favoriteLocation->name, [$user->favoriteLocation->id]) }}
                                            </div>
                                        </div>
                                    @endif
                                    <!-- last login -->
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-9">
                                            {!! ($user->bitrix_token) ? '<i class="fas fa-check text-success" aria-hidden="true"></i> '.trans('general.yes') : '<i class="fas fa-times text-danger" aria-hidden="true"></i> '.trans('general.no')  !!}
                                        </div>
                                    </div>
                                        ...
                                        @if ((Auth::user()->isSuperUser()) && ($snipeSettings->two_factor_enabled!='0') && ($snipeSettings->two_factor_enabled!=''))
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-9" style="margin-top: 10px;">
                                                    ...
                                                    <a class="btn btn-default btn-sm pull-left" id="two_factor_reset"
                                                    ...
                                                    <br>
                                                    <p class="help-block"
                                                       style="line-height: 1.6;">{{ trans('admin/settings/general.two_factor_reset_help') }}</p>
                ...
                "ignoreColumn": ["actions","image","change","checkbox","checkincheckout","icon"]
                    ...
                    "ignoreColumn": ["actions","image","change","checkbox","checkincheckout","delete","download","icon"]
                                ...
                                <tr>
                                    ...
                                    <th data-footer-formatter="sumFormatter"
                                        data-fieldname="purchase_cost">{{ trans('general.purchase_cost') }}</th>
                                ...
                                @foreach ($user->licenses as $license)
                                    <tr>
                                        ...
                                        <td class="hidden-print col-md-2">
                                            @can('update', $license)
                                                <a href="{{ route('licenses.checkin', array('licenseSeatId'=> $license->pivot->id, 'backto'=>'user')) }}"
                                                   class="btn btn-primary btn-sm hidden-print">{{ trans('general.checkin') }}</a>
                                            @endcan
                                        </td>
                                    </tr>
                                @endforeach
                    ...
                    "ignoreColumn": ["actions","image","change","checkbox","checkincheckout","delete","download","icon"]
                                ...
                                <tr>
                                    <th class="col-md-5">{{ trans('general.name') }}</th>
                                    <th class="col-md-6" data-footer-formatter="sumFormatter"
                                        data-fieldname="purchase_cost">{{ trans('general.purchase_cost') }}</th>
                                    <th class="col-md-1 hidden-print">{{ trans('general.action') }}</th>
                                </tr>
                                </thead>
                                    ...
                                    <tr>
                                        ...
                                        <td class="hidden-print">
                                            @can('checkin', $accessory)
                                                <a href="{{ route('accessories.checkin.show', array('accessoryID'=> $accessory->pivot->id, 'backto'=>'user')) }}"
                                                   class="btn btn-primary btn-sm hidden-print">{{ trans('general.checkin') }}</a>
                                            @endcan
                                        </td>
                                    </tr>
                    ...
                    "ignoreColumn": ["actions","image","change","checkbox","checkincheckout","delete","download","icon"]
                                        ...
                                        @foreach ($user->uploads as $file)
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($file->filename)
                                                        @if ( Helper::checkUploadIsImage($file->get_src('users')))
                                                            <a href="{{ route('show/userfile', ['userId' => $user->id, 'fileId' => $file->id, 'download' => 'false']) }}"
                                                               data-toggle="lightbox" data-type="image"><img
                                                                        src="{{ route('show/userfile', ['userId' => $user->id, 'fileId' => $file->id]) }}"
                                                ...
                                                <td>
                                                    @if ($file->filename)
                                                        <a href="{{ route('show/userfile', [$user->id, $file->id]) }}"
                                                ...
                                                <td>
                                                    <a class="btn delete-asset btn-danger btn-sm hidden-print"
                                                       href="{{ route('userfile.destroy', [$user->id, $file->id]) }}"
                ...
                "ignoreColumn": ["actions","image","change","checkbox","checkincheckout","icon"]
                                ...
                                <thead>
                                <tr>
                                    <th data-field="icon" style="width: 40px;" class="hidden-xs"
                                        data-formatter="iconFormatter">Icon
                                    </th>
                                    <th class="col-sm-3" data-field="created_at" data-formatter="dateDisplayFormatter"
                                        data-sortable="true">{{ trans('general.date') }}</th>
                                    <th class="col-sm-2" data-field="admin"
                                    ...
                                    @endif
                                    <th class="col-sm-3" data-field="item"
                                        data-formatter="polymorphicItemFormatter">{{ trans('general.item') }}</th>
                                    <th class="col-sm-2" data-field="target"
