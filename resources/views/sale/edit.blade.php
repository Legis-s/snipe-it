@extends('layouts/edit-form', [
   // 'createText' => trans('admin/hardware/form.create') ,
    'createText' =>"Создать актив на продажу" ,
    'updateText' =>"Обновить актив на продажу",
        ...
        <div class="col-md-7 col-sm-12{{  (\App\Helpers\Helper::checkIfRequired($item, 'asset_tag')) ? ' required' : '' }}">
            @if  ($item->id)
                <input class="form-control" type="text" name="asset_tag" aria-label="asset_tag" id="asset_tag"
                       value="{{ Input::old('asset_tag', $item->asset_tag) }}"/>
            @else
                <input class="form-control" type="text" name="asset_tag" aria-label="asset_tag" id="asset_tag"
                       value="{{ Input::old('asset_tag', \App\Models\Asset::autoincrement_asset()) }}">
            @endif
        ...
        <div class="form-group {{ $errors->has('image_delete') ? 'has-error' : '' }}">
            ...
            <div class="col-md-5">
                <label class="control-label" for="image_delete">
                    <input type="checkbox" value="1" name="image_delete" id="image_delete"
                           class="minimal" {{ Input::old('image_delete') == '1' ? ' checked="checked"' : '' }}>
                ...
                <div style="margin-top: 0.5em">
                    <img src="{{ url('/') }}/uploads/assets/{{ $item->image }}" class="img-responsive"
                         alt="Existing uploaded image thumbnail">
                </div>
            </div>
        </div>
    @endif
