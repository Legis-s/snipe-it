...
//                        'distance' => $distance,
            ...
            $this->info(sprintf('Successfully synchronized %d devices', $count));

            $this->info('Checking for removed devices...');
            ...
            $this->error('MDM configuration is not complete:');
            if ($mdm_url === null) {
            ...
            if ($mdm_password === null) {
                $this->error('- MDM_PASSWORD is not set');
            }
            return 1;
        }
    }
    ...
    public function getToken($client, $mdm_url, $mdm_user, $mdm_password): string
    {
    ...
    public function getDevices($client, $mdm_url, $token)
    {
        $headers = [
            'Content-Type' => 'application/json',
            'Authorization' => 'Bearer ' . $token,
